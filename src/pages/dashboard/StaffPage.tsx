...
import { UserPlus, Clock, Users as UsersIcon, Users, Trash2, ExternalLink, Copy, KeyRound, Pencil, Lock } from 'lucide-react';
...
export default function StaffPage() {
  ...
  const handleEditEmployee = (employee: Employee) => {
    // SECURITY: Don't log employee data
        ...
        <div className="border-t pt-4 space-y-4">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-xl border border-blue-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-semibold text-gray-800">Staff Access Link (Quick Clock In/Out)</p>
            </div>
            <div className="flex flex-wrap gap-2 mb-3">
              <div className="flex-1 min-w-[200px] bg-white border-2 border-blue-200 rounded-lg px-4 py-3 text-sm text-gray-700 font-mono truncate shadow-sm">
                {shopData?.short_code 
                  ? `digiget.uk/s/${shopData.short_code}`
              ...
              <button
                ...
                className="p-2.5 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded-lg transition-colors shadow-sm"
                title="Copy link"
              >
              ...
              <a
                href={shopData?.short_code 
                  ? `https://digiget.uk/s/${shopData.short_code}` 
                  : `${window.location.origin}/staff/${shopId}/clock-in`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-lg transition-colors shadow-sm"
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 bg-green-500 text-white hover:bg-green-600 rounded-lg transition-colors shadow-sm"
          ...
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-5 rounded-xl border border-purple-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-semibold text-gray-800">Staff Portal Link (Full Portal)</p>
            </div>
            <div className="flex flex-wrap gap-2 mb-3">
              <div className="flex-1 min-w-[200px] bg-white border-2 border-purple-200 rounded-lg px-4 py-3 text-sm text-gray-700 font-mono truncate shadow-sm">
                {shopData?.short_code 
                  ? `digiget.uk/staff/${shopData.short_code}`
                  : shopData?.slug 
              ...
              <button
                ...
                className="p-2.5 bg-purple-100 text-purple-700 hover:bg-purple-200 rounded-lg transition-colors shadow-sm"
                title="Copy link"
              >
                <Copy className="w-4 h-4" />
              </button>
              <a
                href={shopData?.short_code 
                  ? `https://digiget.uk/staff/${shopData.short_code}` 
                  : shopData?.slug 
                    ? `${window.location.origin}/staff-portal/${shopData.slug}` 
                    : `${window.location.origin}/staff-portal/${shopId}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 bg-purple-600 text-white hover:bg-purple-700 rounded-lg transition-colors shadow-sm"
                title="Open portal"
              >
                <ExternalLink className="w-4 h-4" />
              </a>
              <a
                href={`https://wa.me/?text=${encodeURIComponent(
                  `📱 Staff Portal Link - Full Access\n\n${shopData?.short_code 
                    ? `https://digiget.uk/staff/${shopData.short_code}` 
                    : shopData?.slug 
                      ? `${window.location.origin}/staff-portal/${shopData.slug}`
                      : `${window.location.origin}/staff-portal/${shopId}`}\n\nAccess your full staff portal with all features!`
                )}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 bg-green-500 text-white hover:bg-green-600 rounded-lg transition-colors shadow-sm"
...
function EmployeeModal({ employee, shopId, onClose, onSave }: EmployeeModalProps) {
  ...
  useEffect(() => {
    // SECURITY: Don't log employee data
    if (employee) {
      // SECURITY: Don't log employee personal data (name, phone, email)
      console.log('Setting form fields from employee (personal data hidden)');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (employee) {
        ...
        // SECURITY: Don't log employee IDs or data
